...
if [ $env = "prod" ]; then
    ...
    #docker run -d -p 5001:5000 -v $dir/certs_prod:/certs -e REGISTRY_HTTP_TLS_CERTIFICATE=/certs/domain.cert -e REGISTRY_HTTP_TLS_KEY=/certs/domain.key --restart=always --name registryprod registry:2    
    docker run -d -p 443:443 -v $dir/certs_prod:/certs -e REGISTRY_HTTP_ADDR=0.0.0.0:443 -e REGISTRY_HTTP_TLS_CERTIFICATE=/certs/domain.cert -e REGISTRY_HTTP_TLS_KEY=/certs/domain.key --restart=always --name registryprod registry:2    
elif [ $env = "hom" ]; then
